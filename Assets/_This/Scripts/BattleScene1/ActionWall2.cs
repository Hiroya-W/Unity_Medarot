...
public class ActionWall2 : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        {
            //フェーズをAttackへ移行
            gameController.phase = GameController2.Phase.Attack;
            gameController.PhaseAttackfunc(0);
        }
        else if (collider.gameObject.CompareTag("Player2"))
        {
            //フェーズをAttackへ移行
            gameController.phase = GameController2.Phase.Attack;
            gameController.PhaseAttackfunc(1);
        }
        else if (collider.gameObject.CompareTag("Player3"))
        {
            //フェーズをAttackへ移行
            gameController.phase = GameController2.Phase.Attack;
            gameController.PhaseAttackfunc(2);
        }
        ...
        {
            //フェーズをDefenceへ移行
            gameController.phase = GameController2.Phase.Defence;
            gameController.PhaseAttackfunc(3);
        }
        else if (collider.gameObject.CompareTag("Enemy2"))
        {
            //フェーズをDefenceへ移行
            gameController.phase = GameController2.Phase.Defence;
            gameController.PhaseAttackfunc(4);
        }
        else if (collider.gameObject.CompareTag("Enemy3"))
        {
            //フェーズをDefenceへ移行
            gameController.phase = GameController2.Phase.Defence;
            gameController.PhaseAttackfunc(5);
        }
    }
}
